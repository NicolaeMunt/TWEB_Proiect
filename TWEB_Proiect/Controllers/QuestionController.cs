...
namespace TWEB_Proiect.Controllers
{
     public class QuestionController : Controller
     {
          ...
          // GET: Question/Details/5 - ОТКРЫТ ДЛЯ ВСЕХ (с загрузкой ответов)
          public ActionResult Details(int id)
          {
               try
               {
                    var question = db.Questions
                        .Include(q => q.QuestionAnswers.Select(a => a.User))
                    ...
                    if (question != null)
                    {
                         ...
                         // Подготавливаем модель для формы ответа (если пользователь авторизован)
                         ViewBag.AnswerModel = new AnswerViewModel
                         {
                              QuestionId = question.Id,
                              QuestionTitle = question.Title,
                              QuestionAuthor = question.Username
                         };

                         // Проверяем, авторизован ли пользователь
                         ViewBag.IsUserLoggedIn = Session["UserId"] != null;
                         ViewBag.CurrentUserId = Session["UserId"];

                         // Сортируем ответы: принятые вверху, потом по дате
